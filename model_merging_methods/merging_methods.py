...
class MergingMethod:
    ...
    def fisher_merging(self, models_to_merge: list, trainers: list, exclude_param_names_regex: list, nums_fisher_examples: list, fisher_scaling_coefficients: list = None,
        ...
        # dictionary of list, where key is the parameter name,
        # value is a list of the corresponding parameters of all the models that need to be merged
        models_to_merge_param_dict = defaultdict(list)  # k是layer名，v是一个list，每个元素是一个模型的对应的那个param
        ...
        for model_idx, (model_to_merge, trainer, num_fisher_examples) in enumerate(zip(models_to_merge, trainers, nums_fisher_examples)):
            ...
            models_to_merge_fisher_weights_list.append(model_to_merge_fisher_weights)  # list里面每一个是一个模型的fisher weights (param name: param value)
    ...
    def regmean_merging(self, models_to_merge: list, train_loaders: list, exclude_param_names_regex: list,
        ...
        :param models_to_merge: list, individual models that need to be merged
        :param train_loaders: list, train_loaders of individual models
        :param exclude_param_names_regex: list, regular expression of names of parameters that need to be excluded
            ...
            for param_name, param_value_list in models_to_merge_param_dict.items():
                ...
                if param_name.endswith(".weight"):
                    ...
                    if module_name in models_to_merge_regmean_weights_list[0].keys():
                        ...
                        inv_sum_module_regmean_weights = torch.pinverse(sum_module_regmean_weights)
        ...
        # iterate each individual model that needs to be merged
        with torch.no_grad():
            for model_idx, (model_to_merge, train_loader, num_regmean_examples) in enumerate(zip(models_to_merge, train_loaders, nums_regmean_examples)):
                param_dict = {param_name: param_value for param_name, param_value in model_to_merge.named_parameters()}
                ...
                if num_regmean_examples % batch_size != 0:
                ...
                for step, inputs in tqdm(enumerate(train_loader), desc=f"computing regmean weights for model {model_idx}"):
                    ...
                    model_to_merge(x, labels=y)  # TODO: this is only for classification

                # move regmean_weights to cpu
                for module_name in regmean_weights.keys():
                    # Tensor, shape (hidden_dim, hidden_dim)
                    regmean_weights[module_name] = regmean_weights[module_name].to("cpu")
    ...
    def ties_merging(self, merged_model: nn.Module, models_to_merge: list, param_value_mask_rate: float = 0.8, scaling_coefficient: float = 1.0):
        """
        ties merging method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        ...
        models_to_merge_task_vectors = [TaskVector(pretrained_model=merged_model, finetuned_model=model_to_merge) for model_to_merge in models_to_merge]
